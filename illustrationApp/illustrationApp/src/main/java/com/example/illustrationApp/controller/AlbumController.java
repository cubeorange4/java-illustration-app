...
public class AlbumController {
	...
	public AlbumController(AlbumRepository albumRepository, AlbumService albumService, AlbumInfoRepository albumInfoRepository, IllustrationRepository illustrationRepository){
	...
	@GetMapping("/album/{id}")
	public String show(@PathVariable(name = "id") Integer albumId, @PageableDefault(page = 0, size = 10, sort = "id") Pageable pageable, Model model) {
		Album album = albumRepository.getReferenceById(albumId);
		Page<AlbumInfo> albumInfo = albumInfoRepository.findByAlbumOrderByCreatedAtDesc(album, pageable);
